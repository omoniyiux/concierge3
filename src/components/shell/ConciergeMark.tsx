...
export function ConciergeWordmark({ className }: { className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2 ${className ?? ""}`}>
      ...
      <span className="text-[15.5px] font-semibold tracking-[-0.02em]">Concierge</span>
    </span>
  );
}
...
export function SiteMark({ name, size = 26 }: { name: string; size?: number }) {
  ...
  return (
    <span
      ...
      className="inline-flex shrink-0 items-center justify-center rounded-[7px] border border-line bg-surface-subtle font-semibold tracking-tight text-text-secondary"
      aria-hidden
    >
      {initials}
    </span>
