    ...
    <div className={cx("overflow-hidden bg-surface shadow-lg", className)} role="img" aria-label={label}>
      {chrome === "app" && (
        <div className="flex items-center gap-2 bg-surface-subtle px-3.5 py-2.5">
          ...
          <span className="ml-2 flex items-center gap-1.5 bg-surface px-2 py-1 text-[10px] text-text-muted">
...
export function SiteBrainFrame() {
  ...
  return (
    <div className="p-5">
      <div className="flex flex-wrap items-center gap-x-7 gap-y-4 bg-surface p-4">
        <div className="flex items-center gap-3.5">
          <RadialGauge value={86} label="Approved knowledge" tone="accent" size={46} />
          <div>
            <p className="text-[11.5px] font-semibold">Ready to answer</p>
        ...
        <dl className="flex gap-6">
          {[
            ...
            <div key={l}>
              ...
              <dd className={cx("t-num mt-1 text-[13.5px]", c)}>{v}</dd>
            </div>
          ))}
        </dl>
      </div>
      ...
      <ul className="mt-3 space-y-3">
        {rows.map((r) => (
          <li
            key={r.title}
            className={cx(
              "flex items-center gap-3 border bg-surface px-3.5 py-2.5",
    ...
    <div className="p-5">
      <div className="overflow-hidden bg-surface">
        <div className="flex items-center gap-2.5 border-b border-divider px-4 py-2.5">
          <span className="flex h-6 w-6 items-center justify-center bg-ink text-[8.5px] font-semibold text-text-inverse">
            C+
          </span>
        ...
        <div className="space-y-3.5 p-4">
          <Bubble side="agent">
            Hi — I can help with appointments, treatments and pricing. What brings you in?
          </Bubble>
          <Bubble side="visitor">Do you do payment plans for Invisalign?</Bubble>
          <div>
            <Bubble side="agent">
              We do. Invisalign starts at $3,900 and financing is available from $149 a month. The free
              consultation includes a 3D scan, so you would get an exact figure before committing.
            </Bubble>
            <p className="mt-1.5 flex flex-wrap items-center gap-2 text-[9.5px] text-text-tertiary">
          ...
          <div>
            ...
            <p className="mt-1.5 flex items-center gap-1.5 text-[9.5px]">
...
function Bubble({ side, children }: { side: "visitor" | "agent"; children: React.ReactNode }) {
  return (
    <div
      className={cx(
        "w-fit max-w-[84%] px-3 py-2 text-[12.5px] leading-[1.55]",
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <p className="text-[12.5px] font-semibold">Maya Robinson</p>
              <Badge tone="restricted">hot</Badge>
            </div>
            <p className="mt-0.5 text-[11.5px] text-text-tertiary">maya.robinson@gmail.com · Austin, TX</p>
          </div>
          <div className="text-right">
            <p className="t-num text-[18px] leading-none">92</p>
            <p className="mt-1 text-[9.5px] text-text-tertiary">lead score</p>
          </div>
        </div>

        <div className="mt-3 h-1.5 w-full overflow-hidden bg-surface-sunken">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2.5">
          {[
            ...
            <div key={k} className="flex items-baseline justify-between gap-3 border-b border-divider pb-1.5">
              <dt className="text-[10px] text-text-tertiary">{k}</dt>
              <dd className="truncate text-[12.5px] font-medium">{v}</dd>
            </div>
          ))}
        </dl>

        <p className="mt-3.5 flex items-center gap-1.5 text-[10px] text-success">
...
export function RoutingFrame() {
  const rules = [
    {
      name: "Emergencies",
      cond: ["Intent", "is", "Wants a person"],
      dest: "Emergency line",
      Icon: PhoneIcon,
    },
    {
      name: "Cosmetic enquiries",
      cond: ["Service", "contains", "veneers"],
      dest: "Cosmetic team",
      Icon: ConversationsIcon,
    },
    {
      name: "High-value leads",
      cond: ["Lead value", "is over", "$3,000"],
      dest: "Front desk",
      Icon: MailIcon,
    },
  ];
      ...
      {rules.map((r, i) => (
        <Card key={r.name} className="p-3.5">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center bg-surface-subtle text-[9.5px] font-semibold tabular-nums text-text-tertiary">
              {i + 1}
            </span>
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                <span className="inline-flex items-center gap-1.5 bg-surface px-2 py-1 text-[10px] font-medium">
  ...
  return (
    <div className="p-5">
      <Card className="border-accent-line bg-accent-subtle p-4">
        ...
        <p className="mt-2 text-[13px] font-semibold leading-[1.3]">
      ...
      <ul className="mt-3 space-y-3">
        {gaps.map(([q, n]) => (
          <li key={q} className="flex items-center gap-3 bg-surface-subtle px-3.5 py-2.5">
            ...
            <span className="shrink-0 bg-ink px-2 py-1 text-[9.5px] font-medium text-text-inverse">
              Answer it
            </span>
          </li>
        ))}
      ...
      <div className="mt-3 bg-surface p-3.5">
...
export function OverviewFrame() {
  ...
  return (
    <div className="flex bg-canvas">
      ...
      <div className="hidden w-[150px] shrink-0 flex-col border-r border-divider bg-surface py-3 sm:flex">
        {[
          ...
          <span
            key={label}
            className={cx(
              "relative mx-2 flex h-7 items-center gap-2 px-2 text-[12px]",
          ...
          >
            ...
            {label}
            {badge && (
              <span className="ml-auto bg-accent-soft px-1.5 text-[9px] font-semibold text-accent-ink">
                {badge}
              </span>
            )}
          </span>
      ...
      <div className="min-w-0 flex-1 p-5">
        ...
        <h3 className="mt-1.5 text-[15px] font-semibold tracking-[-0.02em]">Northlane Dental</h3>

        <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden bg-surface-subtle bg-line sm:grid-cols-4">
          {headline.map((m) => (
            <div key={m.key} className="bg-surface p-3">
              <p className="t-eyebrow text-[9px] text-text-muted">{m.label}</p>
              <p className="t-num mt-1.5 text-[13.5px] leading-none">
                {m.format === "percent" ? `${m.value}%` : m.value.toLocaleString()}
              </p>
              <div className="mt-1.5 flex items-end justify-between">
                <span
                  className={cx("text-[9.5px] font-medium", m.delta > 0 ? "text-success" : "text-danger")}
                >
        ...
        <div className="mt-3 space-y-1.5">
          {[
            ...
            <div key={title} className="flex items-center gap-2.5 bg-surface-subtle px-3 py-2">
              ...
              <span className="shrink-0 text-[9.5px] text-text-muted" aria-hidden>
                →
              </span>
            </div>
          ))}
